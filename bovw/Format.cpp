...
namespace img
{
	Format::Format(const char *fullImageName, HSITable table) 
		:startIndex(0), width(0), height(0), table(table)
	{		
	...
	void Format::loadImageData(){
		...
		try
		{
			...
			std::cout << "El tiempo es: " << t << std::endl;
	...
	void Format::imgFilter2D(double** &image, double** &result)
	{
		...
		//apply the filter
		int x = 0, y = 0;
#pragma omp parallel for private(x, y) schedule(runtime)
		for (x = 0; x < height; x++)
		for (y = 0; y < width; y++)
		{
			double value = 0.0;
